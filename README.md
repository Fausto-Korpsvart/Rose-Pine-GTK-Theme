...
```sh
./install.sh --tweaks moon macos outline float -t green -l
```

> To customise the name of the theme, you can use the `-n` parameter, e.g.

```sh
./install.sh -n "Rosepine-Float-Border" --tweaks outline float
```
...
-n, --name NAME         Specify theme name (Default: Rosepine)
...
--tweaks                Specify versions for tweaks
                        1. [moon]  Moon ColorSchemes version
